...
func (r *resizer) optimizedWidths() (colWidths, rowHeights []int) {
	// Compare against the width including borders: expandTableWidth also
	// accounts for them, and the table is hard-clipped to its requested width
	// after layout. Without this, a table whose content fits the width but
	// whose borders do not takes the expand path, cannot fit, and gets its
	// last column cut off by the final MaxWidth.
